...
class Amasty_Cart_AjaxController extends Mage_Core_Controller_Front_Action
{
   ...
   //reload my cart
    ...
    {   
        $_SERVER['REQUEST_URI'] = str_replace(Mage::getBaseUrl(), '/', (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ''));
...
//reload wishlist
    ...
    private function addToCartResponse($product, $cart, $IsProductView, $params, $text){
       $result = array(
                  ...
                  'b1_name'   =>  $this->__('Vai al Carrello'), 
     ...
     $result['message'] .='</span></p>';
      ...
      // new template
      $result['message'] = '
        <div class="message-ajaxcart">
  <div class="message-ajaxcart-left">
    <h1>AGGIUNTO AL CARRELLO</h1>
    <div class="item-product-cart">
      <div class="img-mcart">
        <img src="'. Mage::helper('catalog/image')->init($product, 'small_image') .'">
      </div>
      <div class="info-mcart">
        <h3>'. Mage::helper('core')->escapeHtml($product->getName()) .'</h3>
        <div class="qty-price">
          '.$this->__('Qty').' '.Mage::app()->getRequest()->getParam('qty').' x <span class="price">'.Mage::helper('checkout')->formatPrice($product->getFinalPrice()) .'</span>
        </div>
      </div>
    </div>
  </div>
  <div class="message-ajaxcart-right">
    <h1>TOTALE PARZIALE: <span class="toltal-price">'.Mage::helper('checkout')->formatPrice($this->getSubtotal($cart)).'</span></h1>
    <div id="btn-mcart">
      <a href="'.Mage::getUrl('checkout/cart').'" class="button btn-change-cart">Vai al Carrello</a>
      <a href="'.Mage::getUrl('firecheckout').'" class="button btn-change-checkout">Paga Ora</a>
    </div>
  </div>
  
  <div id="close-mcart"><i aria-hidden="true" class="fa fa-close"></i></div>
  <script>jQuery("#close-mcart").click(function(){
